...
setTimeout(()=>{
    ...
    {{-- Echo.private('users.{{auth()->user()->id}}')
        .notification((notification)=>{
            ...
            {
                windows.livewire.emit('refresh')
            }
        }); --}}

"

class="flex flex-col transition-all h-full overflow-hidden">{{--   end tag div --}}
    ...
    <header class="px-3 z-10 bg-white sticky top-0 w-full py-2  ">
        ...
        <div class="flex gap-3 items-center p-2 bg-white">
            ...
            <button @click="type='all'" :class="{ 'bg-green-200 border-0 text-black': type == 'all' }"
            ...
            <button @click="type='del'" :class="{ 'bg-red-200 border-0 text-black': type == 'del' }"
                class="inline-flex justify-center items-center rounded-full gap-x-1 text-xs font-medium px-3 lg:px-5 py-1  lg:py-2.5 border ">
                Deleted
            </button>
    ...
    <main class=" overflow-y-scroll overflow-hidden grow  h-full relative " style="contain:content">
        ...
        @foreach ( $conversations as $key => $conversation )
            ...
            <li  id="conversation-{{$conversation->id}}" wire:key="{{$conversation->id}}"
                class="py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-10  transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2  {{ ($conversation->id == $selectedConvrsation?->id )? "bg-gray-200" : ""}}">
                ...
                <aside class="grid grid-cols-12 w-full">
                    <a href="{{route('chat.message',$conversation->id)}}"
